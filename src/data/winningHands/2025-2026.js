...
// CORRECTED 13579 - Hand 1
hands.push({
    name: '13579 - Hand 1 (1 or 3 Suits)',
    value: 25,
    isConcealed: false,
    variations: [
        // One-suit variations (Correct)
        ...suits.map(s => buildVariation([['1', 2, 's1'], ['3', 3, 's1'], ['5', 4, 's1'], ['7', 3, 's1'], ['9', 2, 's1']], [s])),
        // Three-suit variations (Corrected with a specific pattern)
...
[1, 5].forEach(n => {
    hands.push({
        ...
        variations: getPermutations(suits, 3).map(p => buildVariation([
            ...
            [String(n), 3, 's2'], 
            [String(n + 2), 3, 's2'], 
            [String(n + 4), 2, 's3'] // Changed from (n * 2 + 1)
        ], p))
    });
});
...
for (let i = 1; i <= 7; i++) {
    const handName = `Winds-Dragons - Hand 2 (${i}-${i+2})`;
    const variations = [];
    const dragonPerms = getPermutations(['GD', 'RD', 'WD'], 3); // All 6 permutations
    
    suits.forEach(s => {
        dragonPerms.forEach(d_perm => {
            const pattern = [
                ['F', 2],
                [String(i), 1, 's1'],
                [String(i+1), 1, 's1'],
                [String(i+2), 1, 's1'],
                [d_perm[0], 2], // Pair of the 1st dragon in the permutation
                [d_perm[1], 3], // Pung of the 2nd
                [d_perm[2], 4]  // Kong of the 3rd
            ];
            variations.push(buildVariation(pattern, [s]));
        });
    });
    hands.push({ name: handName, value: 25, variations });
}
